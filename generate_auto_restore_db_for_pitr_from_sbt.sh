...
TMP_DIR=/tmp
if [ $# -lt 4 ]; then
        echo "You must provide 3 params.The params tgt_inst_name,"
        echo "rman_channel_params,ctrl_backup_loc and target_data_dir are required. "
        echo "You can use this script likes: \"$0 \\"
        echo " -tgt_inst_name 'oradbdg' \\"
	echo " -rman_channel_params 'NB_ORA_CLIENT=db01,NB_ORA_SERV=nbuserver' \\"
	echo " -ctrl_backup_loc '/cntrl_7289_1_913310485' \\"
...
while [ $# -gt 0 ]
  do
    case $1 in
      ...
      -rman_channel_params) shift; RMAN_CHANNEL_PARAMS=$1; export RMAN_CHANNEL_PARAMS;;  # RMAN_CHANNEL_PARAMS is set
      ...
      -recover_type) shift; RECOVER_TYPE=$1;;  # RECOVER_TYPE is set
      -auto_resetlogs) shift; AUTO_RESETLOGS=$1;;  # AUTO_RESETLOGS is set
    esac;
    shift
  done

if [ -z $RMAN_CHANNEL_PARAMS -o -z $CONTROLFILE_BACKUP_LOC -o -z $TGT_INST_NAME ]; then
        echo "You must provide at least 3 params.The params tgt_inst_name,ctrl_backup_loc and rman_channel_params are required. "
	exit 1
fi
...
GET_TIME_RANGE_SCRIPT=$BASE_DIR/get_time_range.sh
GET_SCN_RANGE_SCRIPT=$BASE_DIR/get_scn_range.sh
...
DB_FILE_NAME_CONVERT=$TMP_DIR/auto_dbfile_name_convert_by_cdshrewd.out
...
REDO_FILE_NAME_CONVERT=$TMP_DIR/auto_redo_file_name_convert_by_cdshrewd.out
...
ONLINE_LOG_DIR=${TMP_DIR}/auto_online_logfile_by_cdshrewd.out
AUTO_RESTORE_SCRIPT=${BASE_DIR}/auto_restore_db_by_cdshrewd.sh
...
if [ $cnt -lt 1 ]; then
	...
	echo "export ORACLE_SID=$TGT_INST_NAME" >>$AUTO_RESTORE_SCRIPT
        if [ -n $RMAN_CHANNEL_PARAMS ]; then
             RMAN_LOG_FILE="$BASE_DIR"/"${RMAN_LOG_PREFIX}"`date +%Y%m%dT%H%M`"${RMAN_LOG_SUFFIX}"
        else
        ...
        for j  in $(seq $cnt )
        do
                echo "allocate channel ch0$j type 'SBT_TAPE';">>$AUTO_RESTORE_SCRIPT
                if [ $j -eq $cnt ]; then
                        echo "SEND '$RMAN_CHANNEL_PARAMS';" >>$AUTO_RESTORE_SCRIPT
                        if [ $RECOVER_TYPE == "time" ]; then
