...
RMAN_LOG_SUFFIX=".out"
RMAN_AUTO_BACKUP_SCRIPT="auto_create_fullbackup_for_pitr_by_cdshrewd.sh"
...
if [ $cnt -lt 1 ]; then
	...
	if [ -e "$PRI_BACKUP_DIR/auto_ctrl_backup_by_cdshrewd.bk" ]; then
           mv "$PRI_BACKUP_DIR/auto_ctrl_backup_by_cdshrewd.bk" "$PRI_BACKUP_DIR/auto_ctrl_backup_by_cdshrewd.`date +%y%m%d%H%M%s`.bk"
        fi
...
for j  in $(seq $cnt )
        do
                ...
                if [ $j -eq $cnt ]; then
                        echo "backup $COMPRESSED database format '$PRI_BACKUP_DIR/auto_fulldb_%U_by_cdshrewd.bk' tag='auto_fullbackup_by_cdshrewd'  plus archivelog;" >>$RMAN_AUTO_BACKUP_SCRIPT
                        echo "backup current controlfile format='$PRI_BACKUP_DIR/auto_ctrl_backup_by_cdshrewd.bk';" >>$RMAN_AUTO_BACKUP_SCRIPT
                fi
        done
